...
class Renderer(object):
        ...
        for x in range(minX, maxX + 1):
            for y in range(minY, maxY + 1):
                ...
                if (0 <= x < self.width) and (0 <= y < self.height):
                    ...
                    P = (x,y)
                    bCoords = lb.barycentricCoords(A, B, C, P)
    ...
    def glCamMatrix(self, translate=(0,0,0), rotate=(0,0,0)):
        ...
        self.viewMatrix= lb.matrix_inverse(self.camMatrix)

    def glLookAt(self, camPos= (0,0,0), eyePos=(0,0,0)):
        worldUp= (0,1,0)

        forward= lb.vector_subtraction(camPos, eyePos)
        forward= lb.normalize_vector(forward)

        right = lb.cross_product(worldUp, forward)
        right= lb.normalize_vector(right)

        up = lb.cross_product(forward, right)
        up= lb.normalize_vector(up)
    ...
    def glModelMatrix(self, translate = (0,0,0), rotate = (0,0,0), scale = (1,1,1)):
        ...
        multMatrix= lb.multi4x4matrix(translation, rotMat)
        result= lb.multi4x4matrix(multMatrix, scaleMat)
    ...
    def glRotationMatrix(self, pitch = 0, yaw = 0, roll = 0):
        ...
        mulMatrix= lb.multi4x4matrix(pitchMat, yawMat)
        result= lb.multi4x4matrix(mulMatrix, rollMat)
        return result
